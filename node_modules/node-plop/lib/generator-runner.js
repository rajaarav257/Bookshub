...
function _default(plopfileApi, flags) {
  ...
  const runGeneratorPrompts = async function (genObject, bypassArr = []) {
    ...
    return await plopfileApi.inquirer.prompt(promptsAfterBypass).then(answers => Object.assign(answers, bypassAnswers));
  ...
  const executeActionLogic = async function (action, cfg, data) {
    ...
    return await Promise.resolve(action(data, cfg, plopfileApi)).then( // show the resolved value in the console
    result => ({
      type,
